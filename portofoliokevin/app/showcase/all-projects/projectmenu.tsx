...
import { easeOut, motion } from "framer-motion"; // Tambah Import
...
export default function ShowcaseMenu() {
  ...
  const containerVariants = {
    ...
    visible: {
      opacity: 1,
      transition: { staggerChildren: 0.1 } // Elemen muncul bergantian
    }
  };

  const itemVariants = {
  hidden: { opacity: 0, y: 40 }, // Naikkan dari 20 ke 40
  visible: { 
    opacity: 1, 
    y: 0, 
    transition: { 
  ...
  return (
    <>
      <div id="projects" className="font-sans w-full max-w-7xl mx-auto px-4 md:px-8 lg:px-12 py-8 overflow-hidden">
        ...
        {/* --- CONTENT SECTION --- */}
        {/* key={activeTab} penting supaya animasi jalan setiap pindah tab */}
        <motion.div
          key={activeTab}
          initial="hidden"
          animate="visible"
          ...
          {activeTab === 'skills' && (
            <div className="">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                ...
                <motion.div variants={itemVariants} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg flex flex-col h-full"
                whileHover={{ 
                y: -5, 
